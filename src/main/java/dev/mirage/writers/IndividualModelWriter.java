...
public class IndividualModelWriter {
    ...
    private List<FieldModel> attachedFields = new ArrayList<>();
    private List<MethodModel> attachedMethods = new ArrayList<>();
    ...
    public void initiateModel() {
        try (UnsignedStreamWrapper wrapper = new UnsignedStreamWrapper(target.getStream())) {
            ...
            mergeFieldModels(attachedFields);
            mergeMethodModels(attachedMethods);

            byte[] fieldBytes = new byte[]{};
            byte[] methodBytes = new byte[]{};

            if (!attachedFields.isEmpty())
                fieldBytes = assembleFields(attachedFields);
            if (!attachedMethods.isEmpty())
                methodBytes = assembleMethods(attachedMethods);
            ...
            if (!attachedFields.isEmpty()) wrapper.write(fieldBytes);
            else wrapper.putUnsignedShort(0);
            if (!attachedMethods.isEmpty()) wrapper.write(methodBytes);
            else wrapper.putUnsignedShort(0);
    ...
    public void attachModel(FieldModel model) {
        attachedFields.add(model);
    }

    public void attachModel(MethodModel model) {
        attachedMethods.add(model);
    }
