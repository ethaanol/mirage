                        ...
                        for (int l = 0; l < codeLength; l++) {
                            ...
                            switch (opcode) {
                                case Opcodes.INVOKESTATIC: {
                                    ...
                                    insns.add(new InstructionModel(Opcodes.INVOKESTATIC, owner, name, desc));
                                    break;
                                }
                                case Opcodes.BIPUSH: {
                                ...
                                case Opcodes.LDC: {
                                    PoolEntry entry = pool.getEntries().get(stream.readUnsignedByte());
                                    bytesRead++;

                                    if (entry instanceof IntegerEntry) {
                                        insns.add(new InstructionModel(Opcodes.LDC, ((IntegerEntry) entry).getValue()));
                                    } else if (entry instanceof StringEntry) {
                                        UTFEntry utfEntry = (UTFEntry) pool.getEntries().get(((StringEntry) entry).getUtfPointer());
                                        insns.add(new InstructionModel(Opcodes.LDC, utfEntry.getValue()));
                                    }
                                    break;
                                }
                                case Opcodes.GOTO: {
                                    ...
                                    insns.add(new InstructionModel(Opcodes.GOTO, label));
                                    method.getLabels().add(label);
                                    break;
                                }
                                case Opcodes.RETURN: {
                                    insns.add(new InstructionModel(Opcodes.RETURN));
                                    break;
                                }
                                case Opcodes.IRETURN: {
                                    insns.add(new InstructionModel(Opcodes.IRETURN));
                                    break;
                                }
                                default:
                                    break;
    ...
    private void readPool(DataInputStream stream, ConstantPool pool, ClassModel model, int cpSize) throws IOException {
        for (int i = 1; i < cpSize; i++) {
            ...
            switch (tag) {
                ...
                case EntryConstants.TAG_METHOD_REF: {
                    int classIndex = stream.readUnsignedShort();
                    int nameAndTypeIndex = stream.readUnsignedShort();

                    pool.addEntry(new MethodRefEntry(classIndex, nameAndTypeIndex));
                    break;
                }
